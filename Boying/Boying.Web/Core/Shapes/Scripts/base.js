(function ($) {
    ...
    $.extend({
        Boying: {
            __cookieName: "Orchrd", // Boying, on a diet
            __cookieExpiration: 180, // roughly 6 months
            ___doCookiePathHack: null,
            __doCookiePathHack: function (path) {
                if ($.Boying.___doCookiePathHack != null) {
                    return $.Boying.___doCookiePathHack;
                }
                var testCookie = "__Boying__";
                $.cookie(testCookie, "1", { path: path });
                $.Boying.___doCookiePathHack = !$.cookie(testCookie);
                $.cookie(testCookie, null);
                return $.Boying.___doCookiePathHack;
            },
            cookie: function (scope, value, options) { // a light-weight wrapper around $.cookie for an Boying.* cookie name
                // because some browsers are weird about the path
                if (options && options.path && $.Boying.__doCookiePathHack(options.path)) {
                    options.path = options.path.substring(0, options.path.lastIndexOf("/"));
                }
                return $.cookie($.Boying.__cookieName + (scope ? scope.toLowerCase() : ""), value, options);
            },
            cookiesInTheBoying: function () {
                return $.Boying.cookiesLike($.Boying.__cookieName);
            },
            ...
            setting: function (name, value, options) { // cookie-stored settings (only, at the moment)
                ...
                if (typeof value === "undefined") { // try to get the setting value from the default "root" cookie
                    var cookies = $.Boying.cookiesInTheBoying();
                    for (var i = 0; i < cookies.length; i++) {
                    ...
                    var cookie = $.Boying.cookie(scope);
                    ...
                    $.Boying.cookie(scope, dataString, { expires: $.Boying.__cookieExpiration, path: (options && options.path) || "/" }); // todo: default path should be app path
                }
            }
        }
    });
