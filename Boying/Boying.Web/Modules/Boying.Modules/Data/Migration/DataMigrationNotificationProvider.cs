...
using Boying.Modules.Extensions;
using Boying.UI.Admin.Notification;
using Boying.UI.Notify;

namespace Boying.Modules.Data.Migration
{
    /// <summary>
    ...
    /// </summary>
    [BoyingSuppressDependency("Boying.Data.Migration.DataMigrationNotificationProvider")]
            ...
            if (features.Any())
            {
                ...
                yield return new NotifyEntry
                {
                    ...
                    T(string.Join(", ", features
                        .Select(feature =>
                            string.Format("<a href=\"{0}#{1}\">{2}</a>", urlHelper.Action("Features", "Admin", new RouteValueDictionary { { "area", "Boying.Modules" } }), feature.AsFeatureId(n => T(n)), feature))))),
