...
using Boying.Core.Users.Events;
using Boying.Core.Users.Models;
using Boying.Data;
...
namespace Boying.Core.Users.Services
{
    ...
    public class MembershipService : IMembershipService
    {
        ...
        private readonly IRepository<RegistrationSettingsRecord> _registrationSettingRepository;
        ...
        public MembershipService(
            ...
            IRepository<RegistrationSettingsRecord> registrationSettingRepository,
            IRepository<UserRecord> userRepository)
        {
            ...
            _registrationSettingRepository = registrationSettingRepository;
        ...
        {
            Logger.Information("CreateUser {0} {1}", createUserParams.Mobile, createUserParams.Email);

            var registrationSettings = _registrationSettingRepository.Get(c => c != null) ?? new RegistrationSettingsRecord();

            var user = new UserPart
            {
        ...
        public IUser GetUser(string mobile)
        {
            ...
            var userRecord = _userRepository.Get(c => c.Mobile == clearMobile);
        ...
        private static UserPart UserConvert(UserRecord userRecord)
        {
            return new UserPart
            {
                CreatedUtc = userRecord.CreatedUtc,
                Email = userRecord.Email,
                EmailChallengeToken = userRecord.EmailChallengeToken,
                RegistrationStatus = userRecord.RegistrationStatus,
                EmailStatus = userRecord.EmailStatus,
                HashAlgorithm = userRecord.HashAlgorithm,
                Id = userRecord.Id,
                LastLoginUtc = userRecord.LastLoginUtc,
                LastLogoutUtc = userRecord.LastLogoutUtc,
                Mobile = userRecord.Mobile,
                NickName = userRecord.NickName,
                Password = userRecord.Password,
                PasswordFormat = userRecord.PasswordFormat,
                PasswordSalt = userRecord.PasswordSalt,
            };
        }

        public IUser ValidateUser(string mobileOrEmail, string password)
        {
            var lowerName = mobileOrEmail == null ? "" : mobileOrEmail.ToLowerInvariant();

            var userRecord = _userRepository.Get(c => c.Mobile == lowerName);

            if (userRecord == null)
                userRecord = _userRepository.Get(c => c.Email == lowerName);

            if (userRecord == null)
                return null;

            var user = UserConvert(userRecord);
        ...
        private bool ValidatePasswordHashed(UserPart userPart, string password)
        {
            ...
            {
                var keepOldConfiguration = _appConfigurationAccessor.GetConfiguration("Boying.Users.KeepOldPasswordHash");
