...
using Boying.ResponseFiles;

namespace Boying.Host
{
    /// <summary>
    ...
    /// between the command line and the CommandHostAgent, which is known to the Boying
    ...
    public class CommandHost : MarshalByRefObject, IRegisteredObject
    {
        ...
        {
            // Boying objects passed as parameters into this AppDomain should derive from MarshalByRefObject and have
        ...
        public CommandReturnCodes RunCommand(TextReader input, TextWriter output, Logger logger, BoyingParameters args)
        {
            ExtendLifeTimeLeases(input, output);
        ...
        public CommandReturnCodes RunCommandInSession(TextReader input, TextWriter output, Logger logger, BoyingParameters args)
        {
            ExtendLifeTimeLeases(input, output);
        ...
        public CommandReturnCodes RunCommands(TextReader input, TextWriter output, Logger logger, IEnumerable<ResponseLine> responseLines)
        {
            ...
            foreach (var line in responseLines)
            {
                ...
                var args = new BoyingParametersParser().Parse(new CommandParametersParser().Parse(line.Args));
        ...
        {
            return Activator.CreateInstance("Boying.Framework", "Boying.Commands.CommandHostAgent").Unwrap();
        }
