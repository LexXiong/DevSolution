...
using System.Linq;
using Boying.Host;
using Boying.HostContext;
using Boying.Parameters;

namespace Boying
{
    internal class BoyingHost
    {
        private readonly TextReader _input;
        private readonly TextWriter _output;
        private readonly ICommandHostContextProvider _commandHostContextProvider;

        public BoyingHost(TextReader input, TextWriter output, string[] args)
        {
            _input = input;
            _output = output;
        ...
        {
            _output.WriteLine("Initializing Boying session. (This might take a few seconds...)");
            ...
            {
                _output.WriteLine("Failed to initialize Boying session.");
            }
        ...
        private CommandReturnCodes RunCommandInSession(CommandHostContext context, string command)
        {
            try
            {
                var args = new BoyingParametersParser().Parse(new CommandParametersParser().Parse(new CommandLineParser().Parse(command)));
                return context.CommandHost.RunCommandInSession(_input, _output, context.Logger, args);
            }
            catch (AppDomainUnloadedException)
            {
                _output.WriteLine("AppDomain of Boying session has been unloaded. (Retrying...)");
        ...
        {
            _output.WriteLine("The Boying command interpreter supports running a few built-in commands");
            _output.WriteLine("as well as specific commands from enabled features of an Boying installation.");
            _output.WriteLine("");
        ...
        {
            _output.WriteLine("Executes Boying commands from a Boying installation directory.");
            _output.WriteLine("");
            _output.WriteLine("Usage:");
            _output.WriteLine("   Boying.exe");
            _output.WriteLine("       Starts the Boying command interpreter");
            _output.WriteLine("");
            _output.WriteLine("   Boying.exe <command-name> [arg1] ... [argn] [/switch1[:value1]] ... [/switchn[:valuen]]");
            _output.WriteLine("       Executes a single command");
            _output.WriteLine("");
            _output.WriteLine("   Boying.exe @response-file1 ... [@response-filen] [/switch1[:value1]] ... [/switchn[:valuen]]");
            ...
            _output.WriteLine("       Specifies the Boying installation directory. The current directory is the default.");
            _output.WriteLine("");
            _output.WriteLine("   /Verbose");
            _output.WriteLine("   /v");
