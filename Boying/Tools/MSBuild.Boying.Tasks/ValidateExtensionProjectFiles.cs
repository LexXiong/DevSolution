...
namespace MSBuild.Boying.Tasks
{
    ...
    public class ValidateExtensionProjectFiles : Task
    {
        ...
        public class Validator
        {
            ...
            {
                const string BoyingbasiccorrectnessRuleset = "BoyingBasicCorrectness.ruleset";
                ...
                foreach (var element in elements)
                {
                    ...
                    bool isValid = StringComparer.OrdinalIgnoreCase.Equals(filename, BoyingbasiccorrectnessRuleset);
                    if (!isValid)
                    {
                        string message = string.Format(
                            "\"{0}\" element should be \"{1}\" instead of \"{2}\".",
                            element.Name.LocalName, BoyingbasiccorrectnessRuleset, element.Value);
                        AddValidationError(element, message);
                    }
                }
            }
