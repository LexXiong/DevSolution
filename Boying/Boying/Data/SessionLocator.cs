...
namespace Boying.Data
{
    ...
    public class TransactionManager : ITransactionManager, IDisposable
    {
        ...
        private void EnsureSession(IsolationLevel level)
        {
            ...
            _session = sessionFactory.OpenSession(new BoyingSessionInterceptor(_interceptors.ToArray(), Logger));
        ...
        private class BoyingSessionInterceptor : IInterceptor
        {
            private readonly ISessionInterceptor[] _interceptors;
            private readonly ILogger _logger;

            public BoyingSessionInterceptor(ISessionInterceptor[] interceptors, ILogger logger)
            {
                _interceptors = interceptors;
                _logger = logger;
            }
            ...
            {
                return ex is BoyingSecurityException || !ex.IsFatal();
            }
        }
    }
}