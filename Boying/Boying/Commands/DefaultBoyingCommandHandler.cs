...
{
    public abstract class DefaultBoyingCommandHandler : ICommandHandler
    {
        protected DefaultBoyingCommandHandler()
        {
        ...
        private void SetSwitchValue(KeyValuePair<string, string> commandSwitch)
        {
            ...
            if (propertyInfo.GetCustomAttributes(typeof(BoyingSwitchAttribute), false).Length == 0)
            {
                throw new InvalidOperationException(T("A property \"{0}\" exists but is not decorated with \"{1}\"", commandSwitch.Key, typeof(BoyingSwitchAttribute).Name).Text);
            }
        ...
        private void CheckMethodForSwitches(MethodInfo methodInfo, IDictionary<string, string> switches)
        {
            ...
            foreach (BoyingSwitchesAttribute switchesAttribute in methodInfo.GetCustomAttributes(typeof(BoyingSwitchesAttribute), false))
            {
