...
namespace Boying.Environment.Features
{
                    ...
                    (currentFeatureId, featuresState) =>
                    {
                        ...
                        return feature.Key.Dependencies
                                      .Select(fId =>
                                      {
                                          ...
                                          if (states.Count == 0)
                                          {
                                              throw new BoyingException(T("Failed to get state for feature {0}", fId));
                                          }

                                          if (states.Count > 1)
                                          {
                                              throw new BoyingException(T("Found {0} states for feature {1}", states.Count, fId));
                                          }
