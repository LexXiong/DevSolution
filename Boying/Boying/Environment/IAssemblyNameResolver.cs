...
namespace Boying.Environment
{
    ...
    public class BoyingFrameworkAssemblyNameResolver : IAssemblyNameResolver
    {
        private readonly ICacheManager _cacheManager;

        public BoyingFrameworkAssemblyNameResolver(ICacheManager cacheManager)
        {
        ...
        public string Resolve(string shortName)
        {
            // A few common .net framework assemblies are referenced by the Boying.Framework assembly.
            // Look into those to see if we can find the assembly we are looking for.
            var BoyingFrameworkReferences = _cacheManager.Get(typeof(IAssemblyLoader), true, ctx =>
                            ctx.Key.Assembly
            ...
            if (BoyingFrameworkReferences.TryGetValue(shortName, out assemblyName))
            {
