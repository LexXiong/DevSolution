...
using Boying.Mvc;
using Boying.Mvc.Extensions;
using Boying.Utility;
using Boying.Utility.Extensions;

namespace Boying.Environment
{
    // All the event handlers that DefaultBoyingHost implements have to be declared in BoyingStarter.
    public class DefaultBoyingHost : IBoyingHost, IShellSettingsManagerEventHandler, IShellDescriptorManagerEventHandler
    {
            ...
            IHttpContextAccessor httpContextAccessor)
        {
            ...
            _tenantsToRestart = new ContextState<IList<ShellSettings>>("DefaultBoyingHost.TenantsToRestart", () => new List<ShellSettings>());
        ...
        void IBoyingHost.ReloadExtensions()
        {
            DisposeShellContext();
        }

        void IBoyingHost.BeginRequest()
        {
            Logger.Debug("BeginRequest");
            BeginRequest();
        }

        void IBoyingHost.EndRequest()
        {
            Logger.Debug("EndRequest");
            EndRequest();
        }

        IWorkContextScope IBoyingHost.CreateStandaloneEnvironment(ShellSettings shellSettings)
        {
        ...
        private void MonitorExtensions()
        {
            ...
            _cacheManager.Get("BoyingHost_Extensions", true,
                              ctx =>
                              {
