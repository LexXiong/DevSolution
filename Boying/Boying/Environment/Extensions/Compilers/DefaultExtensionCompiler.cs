    ...
    public class DefaultExtensionCompiler : IExtensionCompiler
    {
        ...
        public void Compile(CompileExtensionContext context)
        {
            ...
            try
            {
                ...
                foreach (var assemblyReference in projectFileDescriptor.References)
                {
                    ...
                    else
                    {
                        ...
                        throw new BoyingCoreException(T("Assembly reference '{0}' for project '{1}' cannot be loaded", assemblyReference.FullName, context.VirtualPath));
                    }
                }
            }
            catch (Exception ex)
            {
                ...
                throw new BoyingCoreException(T("Error compiling module \"{0}\" from file \"{1}\":\r\n{2}", moduleName, context.VirtualPath, ex.Message), ex);
            }
        }
