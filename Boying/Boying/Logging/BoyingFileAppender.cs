...
{
    public class BoyingFileAppender : RollingFileAppender
    {
        /// <summary>
            ...
            lock (this)
            {
                ...
                for (int i = 1; !fileOpened && i <= Retries; i++)
                {
                    ...
                    catch
                    {
                        ...
                        LogLog.Error(typeof(BoyingFileAppender), string.Format("BoyingFileAppender: Failed to open [{0}]. Attempting [{1}-{2}] instead.", fileName, fileName, newSuffix));
                    }
                }
